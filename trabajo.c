...
/*void MostrarProductos(ventas vender[],int num) {
  int i; 
	 for(i=0;i<num;i++)  {
	    printf("%d: ",i); //Imprime que posicion ocupa el producto                             //
		MostrarProductos(vender[i]);  
		}  
		} 
		
void BuscarProducto(ventas vender[],int num,ventas x, int campo) {
  int i;
	  for(i=0;i<num;i++)  {
	     if(0==Compara(vender[i],x,campo)) //se ha encontrado 1 coincidenc.   
		 {    
		 printf("%d: ",i);
		  //mostramos el numero de contacto    
		MostrarContacto(vender[i]);//y luego el contacto   
		  }  
		} 
}
