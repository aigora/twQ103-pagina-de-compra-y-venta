...
int LeerPagina(char fichero[], producto pagina[], int* num)
{
	producto prod;
	int n=0;
...
int BuscarProducto (producto [], float precio, producto p, int campo)
{
	int i;
	ImprimeCabecera();
	for(i=0; i<num;i++)
	{
		if(0==Compara(pagina[i],p,campo))
		{
			printf("%d: ", i);
			MostrarProducto(pagina[i]);
		}
	}
	ImprimePie();
}
...
int MostrarProducto (producto pagina[], int num)
{
	int i;
	ImprimeCabecera();
	for(i=0; i<num;i++)
